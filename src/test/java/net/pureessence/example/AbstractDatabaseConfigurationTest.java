...
public abstract class AbstractDatabaseConfigurationTest {
  ...
  public static void setUpTestSuite() throws Exception {
    ...
    statement
        .executeUpdate("create table configuration(key varchar(255), value varchar(255), application varchar(255))");
    // load alpha properties
    statement.executeUpdate("insert into configuration(key, value, application) values ('first.name', '"
        + APPLICATION_ONE_FIRST_NAME + "', '" + Constants.APPLICATION_ONE + "')");
    statement.executeUpdate("insert into configuration(key, value, application) values ('last.name', '"
        + APPLICATION_ONE_LAST_NAME + "', '" + Constants.APPLICATION_ONE + "')");
    statement.executeUpdate("insert into configuration(key, value, application) values ('email', '"
        + APPLICATION_ONE_EMAIL + "', '" + Constants.APPLICATION_ONE + "')");
    // load genesis properties
    statement.executeUpdate("insert into configuration(key, value, application) values ('first.name', '"
        + APPLICATION_TWO_FIRST_NAME + "', '" + Constants.APPLICATION_TWO + "')");
    statement.executeUpdate("insert into configuration(key, value, application) values ('last.name', '"
        + APPLICATION_TWO_LAST_NAME + "', '" + Constants.APPLICATION_TWO + "')");
    statement.executeUpdate("insert into configuration(key, value, application) values ('email', '"
        + APPLICATION_TWO_EMAIL + "', '" + Constants.APPLICATION_TWO + "')");
    conn.commit();
    conn.close();
  }
  ...
  private static Connection getConnection() throws Exception {
    ...
    return DriverManager.getConnection(getDatabaseForConfigurationProperty("url"),
        getDatabaseForConfigurationProperty("username"), getDatabaseForConfigurationProperty("password"));
  }
